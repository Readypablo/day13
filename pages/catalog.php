...
$first_name=$_SESSION['user_name_us'];
$sql5 = 'SELECT * from nots';
$result = $con->query($sql5);
while ($row = $result->fetch_assoc()) {
    $idp= $row['id'];
    echo '
    <div class="popular-card">
    <img src="../img/nots/'.$row['img'].'" alt="" class="imgbooks">
         <h2>'.$row['first_namu'].'</h2>
         <h2>цена: '. $row['cost'].'$</h2>';
 
         echo ' <form method="post"><button type="submit" value='.$idp.' class="btn-del" name='.$idp.'> В корзину </button></form>';

         if(isset($_POST["$idp"])){
            $kkz=$_POST["$idp"];

          $sqlp = "INSERT INTO `workoutnot` (first_nameee, not_id) VALUES ('$first_name', '$kkz')";
          mysqli_query($con, $sqlp) or die(mysqli_error($con));
         }
